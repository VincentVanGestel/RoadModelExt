...
/**
 * The snapshot for a {@link CachedDynamicGraphRoadModel}.
 * It is used by a caching enabled road model to save time
 * on calculating shortest path.
 * @author Vincent Van Gestel
 */
@AutoValue
abstract class CachedDynamicGraphRoadModelSnapshot
    implements RoadModelSnapshot {

	CachedDynamicGraphRoadModelSnapshot() {}
  ...
  static CachedDynamicGraphRoadModelSnapshot create(
      ImmutableGraph<ConnectionData> graph, RoutingTable routingTable, Unit<Length> distanceUnit) {
    return new AutoValue_CachedDynamicGraphRoadModelSnapshot(graph, routingTable, distanceUnit);
  }
